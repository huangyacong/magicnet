...
bool SeAddListenSocket(struct SENETCORE *pkNetCore, int iDomain, HSOCKET kHScoket, SOCKET kSocket)
{
	...
	pkListenSocket->iDomain = iDomain;
...
void SeCreateTimer(struct SENETCORE *pkNetCore, int iMillSec)
{
	...
	CreateTimerQueueTimer(&pkNetCore->kTimerHandle, NULL, SeTimerRoutine, &pkNetCore->kHandle, SENETCORE_TIMER_FIRST <= 0 ? 1 : SENETCORE_TIMER_FIRST, iMillSec, WT_EXECUTEDEFAULT);
}
...
bool SeNetCoreAcceptEx(struct SENETCORE *pkNetCore, int iDomain, HSOCKET kListenHSocket, SOCKET kSocket)
{
	...
	socket = SeSocket(iDomain, SOCK_STREAM);
...
void SeSetAcceptSocket(struct SENETCORE *pkNetCore)
{
	...
	if (SeNetCoreAcceptEx(pkNetCore, pkListenSocket->iDomain, pkListenSocket->kHScoket, pkListenSocket->kSocket))
	{
	...
	int iHeaderLen, bool bNoDelay, int iTimeOut, SEGETHEADERLENFUN pkGetHeaderLenFun, SESETHEADERLENFUN pkSetHeaderLenFun)
{
	...
	SeAddListenSocket(pkNetCore, domain, kHSocket, socket);
...
bool SeNetCoreSendBuf(struct SENETCORE *pkNetCore, struct SESOCKET *pkNetSocket)
{
	...
	if(!SeNetSreamRead(&pkNetSocket->kSendNetStream, &(pkNetCore->kSocketMgr.kNetStreamIdle), pkNetSocket->pkGetHeaderLenFun, 0, pkIOData->kBuf.buf, &iSize, 0))
	{
...
void SeNetCoreListenSocket(struct SENETCORE *pkNetCore, struct SESOCKET *pkNetSocketListen, SOCKET kSocket, struct IODATA *pkIOData)
{
	...
	SeSetAddrToBuf(pkNetSocket->iDomain, remote_addr, pkNetSocket->acIPAddr, (int)sizeof(pkNetSocket->acIPAddr), &pkNetSocket->iIPPort);
	SeNetSocketMgrAddSendOrRecvInList(&pkNetCore->kSocketMgr, pkNetSocket, true);

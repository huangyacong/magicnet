...
void SeListHeadAdd(struct SELIST *root, struct SENODE *node)
{
	...
	assert(!node->next);
#if defined(_TEST)
	if (SeListNodeInRoot(root, node) == 1)
	{
		assert(0 != 0);
	}
#endif
...
struct SENODE *SeListRemove(struct SELIST *root, struct SENODE *node)
{
	assert(root && node);
#if defined(_TEST)
	if (SeListNodeInRoot(root, node) == 0)
	{
		assert(0 != 0);
	}
#endif
...
struct SENODE *SeListRemoveEnd(struct SELIST *root, struct SENODE *node_end)
{
...
#if defined(_TEST)
	if (SeListNodeInRoot(root, node_end) == 0)
	{
		assert(0 != 0);
	}
#endif
	...
	if (node_end->next)
	{
		root->head = node_end->next;
		node_end->next->prev = 0;
	}
	else
	{
		root->head = 0;
